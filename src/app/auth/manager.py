...
class AuthManager:
    ...
    async def _create_tokens_for_user(self, session: AsyncSession, user: User) -> schemas.TokenResponse:
        ...
        seller_id = None
        if user.is_seller:
            seller = await self.sellers_service.get_seller_by_master_id(
                session, user.id
            )
            if seller:
                seller_id = seller.id
        ...
        access_token = self.jwt_utils.create_access_token(user, seller_id=seller_id)
        
        # Create refresh token in database (UUID generated by DB)
        expires_at = datetime.now(timezone.utc) + self.jwt_utils.refresh_token_ttl
