...
class YooKassaClient:
    ...
    async def create_payment(
        ...
        try:
            ...
            await logger.info(
                ...
                extra={"payment_id": data.get("id"), "amount": amount, "currency": currency}
            )
    ...
    async def get_payment(self, payment_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            await logger.error(f"Error getting payment: {str(e)}", extra={"payment_id": payment_id})
            raise
    ...
    async def cancel_payment(
        ...
        try:
            ...
            data = response.json()
            await logger.info(
                f"Payment canceled successfully",
            ...
            await logger.error(f"Error canceling payment: {str(e)}", extra={"payment_id": payment_id})
            raise
    ...
    async def get_webhooks(self) -> Dict[str, Any]:
        ...
        try:
            ...
            await logger.error(
                f"Failed to get webhooks: {e.response.status_code}",
                extra=error_details
            )
            raise
        except Exception as e:
    ...
    async def create_webhook(
        ...
        try:
            response = await self.client.post(url_endpoint, json=webhook_data, headers=headers)
            response.raise_for_status()
            
            data = response.json()
            await logger.info(
                f"Webhook created successfully",
                extra={"event": event, "url": url}
            )
            ...
            await logger.error(f"Error creating webhook: {str(e)}", extra={"event": event, "url": url})
            raise
